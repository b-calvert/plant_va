...
from plant_va.plant_va_config.presets import (
    DEFAULT_DATA, DEFAULT_LABELS, VALENCE_WINDOW, DEFAULT_CV
)
...
def main(data=DEFAULT_DATA, labels=DEFAULT_LABELS, window=VALENCE_WINDOW, cv=DEFAULT_CV):
    ...
    # --- REAL performance (unshifted labels) ---
    real_acc = float(np.mean(accs))
    real_bacc = float(np.mean(baccs))
    real_f1 = float(np.mean(f1s))
    print(f"\nREAL bacc (reported): {real_bacc:.3f}")

    # For the null, consider fewer splits if needed
    n_splits_null = cv.n_splits  # try this first
    # n_splits_null = min(3, cv.n_splits)  # uncomment if you hit single-class fold errors

    for min_shift_minutes, seed in [(360, 0), (720, 1)]:  # 6h, 12h
        null_bacc, min_shift_windows = time_shift_null(
            Xw=Xw,
            y=y,
            n_splits=n_splits_null,
            stride_minutes=window.stride_minutes,
        ...
        print(f"\nVALENCE TIME-SHIFT NULL (>= {min_shift_minutes} min):")
